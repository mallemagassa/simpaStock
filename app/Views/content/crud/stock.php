        ...
        <div class="p-4 bg-white block sm:flex items-center my-4 justify-between border-b border-gray-200 lg:mt-1.5 dark:bg-gray-800 dark:border-gray-700">
            <div class="w-full mb-1">
                <div class="items-center justify-end block sm:flex md:divide-x md:divide-gray-100 dark:divide-gray-700">
                    <?php if (!$user->inGroup('agent-sortie')) :?>
            ...
            <div class="overflow-x-auto">
                ...
                <div class="flex items-center mb-4 sm:mb-2">
                    <form method="get" action="<?= base_url('stock') ?>" class="">   
                    ...
                    <div class="overflow-hidden shadow">
                        <table  class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                            ...
                            <?php if ($user->can('show.stock')  || $user->can('in.stock') || $user->can('out.stock')) :?>
                                    ...
                                    <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                                        ...
                                        <td class="p-4 space-x-2 whitespace-nowrap">
                                            ...
                                            <div id="<?= esc($value['id']) ?>dropdownDots" class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700 dark:divide-gray-600">
                                                ...
                                                <?php if ($user->can('in.stock')) :?>
                                                    <li>
                                                        <button 
                                                            type="button" 
                                                            data-id="<?=esc($value['id'])?>" 
                                                            data-quantity="<?=esc($value['quantity'])?>" 
                                                            data-modal-target="in-stock-modal" 
                                                            data-modal-toggle="in-stock-modal" 
                                                            class="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800"
                                                            onclick="openInStockModal(<?=esc($value['quantity'])?>)">
                                                            Entrée
                                                        </button>
                                                    </li>
                                                <?php endif?>
                                                <?php if ($user->can('edit.stock')) :?>
                        ...
                        <?= $pager->links('stocks','tailwind_pagination') ?>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="overflow-x-auto">
                <div class="inline-block min-w-full align-middle">
                    ...
                    <div class="flex justify-end mx-4 my-4">
                        <form method="post" action="/filterStock" class="flex items-center space-x-2">
                    ...
                    <div class="flex justify-center my-4">
                        <form method="post" action="/filterStocks">
...
<div id="drawer-update-stock-default" class="fixed top-0 right-0 z-40 w-full h-screen max-w-xs p-4 overflow-y-auto transition-transform <?= session('errors') ? 'translate-x-0' : 'translate-x-full' ?> bg-white dark:bg-gray-800" tabindex="-1" aria-labelledby="drawer-label" aria-hidden="true">
    ...
    <form action="<?= base_url('stock/update') ?>" method="POST">
        ...
        <div>
            <label for="purchase_price" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Prix d'Inventaire:</label>
    ...
    <div class="relative w-full h-full max-w-2xl px-4 md:h-auto">
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-800">
            <div class="flex items-start justify-between p-5 border-b rounded-t dark:border-gray-700">
                <h3 class="text-xl font-semibold dark:text-white">Entrée Stock</h3>
            ...
            <div class="p-6 space-y-6">
                <form action="" method="POST">
                    ...
                    <!-- Quantité actuelle -->
                    <div>
                        <label for="current_quantity" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Quantité actuelle :</label>
                        <input type="number" disabled id="current_quantity" class="bg-gray-50 border <?= session('errors.quantity') ? 'border-red-500' : 'border-gray-300' ?> text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" required>
                    </div>

                    <!-- Quantité entrée -->
                    <div>
                        <label for="entry_quantity" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nombre de quantité Entrée :</label>
                        <input type="number" id="entry_quantity" name="quantity" class="bg-gray-50 border <?= session('errors.quantity') ? 'border-red-500' : 'border-gray-300' ?> text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="0" required oninput="updateNextQuantity()">
                    </div>

                    <!-- Quantité après entrée -->
                    <div>
                        <label for="next_quantity" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Quantité Après Entrée :</label>
                        <input type="number" id="next_quantity" disabled class="bg-gray-50 border <?= session('errors.quantity') ? 'border-red-500' : 'border-gray-300' ?> text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" required>
                    </div>
                ...
                <div class="items-center p-6 border-t border-gray-200 rounded-b dark:border-gray-700">
                    <button class="text-white bg-primary-700 hover:bg-primary-800 font-medium rounded-lg text-sm px-5 py-2.5" type="submit">Valider Entrée</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    function openInStockModal(quantity) {
        document.getElementById('current_quantity').value = quantity;
        document.getElementById('entry_quantity').value = '';
        document.getElementById('next_quantity').value = '';
    }

    function updateNextQuantity() {
        const currentQuantity = parseInt(document.getElementById('current_quantity').value) || 0;
        const entryQuantity = parseInt(document.getElementById('entry_quantity').value) || 0;
        document.getElementById('next_quantity').value = currentQuantity + entryQuantity;
    }
</script>
