    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center justify-start">
        ...
        <a href="<?= base_url() ?>" class="flex ml-2 md:mr-24">
          <img src="<?= base_url('assets/images/logo/logo.png') ?>" class="h-8 mr-3" alt="FlowBite Logo" />
      ...
      <div class="flex items-center">
          ...
          <!-- Notifications -->
          <button id="mark-read-btn" type="button" data-dropdown-toggle="notification-dropdown" class="relative p-2 text-gray-500 rounded-lg hover:text-gray-900 hover:bg-gray-100 dark:text-gray-400 dark:hover:text-white dark:hover:bg-gray-700">
            ...
            <!-- Badge -->
            <?php if (sizeof($notificationsNoRead) > 0 ) : ?>
            <span class="absolute top-0 right-0 inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-white bg-red-600 rounded-full"> <?= sizeof($notificationsNoRead) ?></span>
            <?php endif ?>
          </button>
         ...
         <!-- Dropdown menu -->
          <div class="z-20 z-50 hidden max-w-sm my-4 overflow-hidden text-base list-none bg-white divide-y divide-gray-100 rounded shadow-lg dark:divide-gray-600 dark:bg-gray-700" id="notification-dropdown">
              ...
              <div>
                <?php foreach ($notifications as $value) : ?>
                    <div class="flex items-center justify-between px-4 py-3 border-b hover:bg-gray-100 dark:hover:bg-gray-600 dark:border-gray-600">
                        <div class="w-full pl-3">
                            <div class="text-gray-500 font-normal text-sm mb-1.5 dark:text-gray-400"><?= esc($value["message"]) ?></div>
                            <div class="text-xs font-medium text-primary-700 dark:text-primary-400"><?= esc($value["created_at"]) ?></div>
                        </div>
                        <!-- Bouton de suppression -->
                        <button class="delete-notification-btn p-2 text-red-600 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full" data-id="<?= esc($value['id']) ?>">
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                <path fill-rule="evenodd" d="M10 18a3 3 0 01-3-3H6a1 1 0 010-2h8a1 1 0 010 2h-1a3 3 0 01-3 3zM5 8h10v8H5V8zm7-4H8v2h4V4z" clip-rule="evenodd"></path>
                            </svg>
                        </button>
                    </div>
                <?php endforeach ?>
          ...
          <div class="z-20 z-50 hidden max-w-sm my-4 overflow-hidden text-base list-none bg-white divide-y divide-gray-100 rounded shadow-lg dark:bg-gray-700 dark:divide-gray-600" id="apps-dropdown">
            ...
            <div class="grid grid-cols-3 gap-4 p-4">
              <a href="<?= base_url('user') ?>" class="block p-4 text-center rounded-lg hover:bg-gray-100 dark:hover:bg-gray-600">
                ...
                <div class="text-sm font-medium text-gray-900 dark:text-white">Utilisateurs</div>
              </a>
              <!--
              ...
              <a href="<?= base_url('product') ?>" class="block p-4 text-center rounded-lg hover:bg-gray-100 dark:hover:bg-gray-600">
                ...
                <div class="text-sm font-medium text-gray-900 dark:text-white">Produits</div>
              </a>
              <a href="<?= base_url('stock') ?>" class="block p-4 text-center rounded-lg hover:bg-gray-100 dark:hover:bg-gray-600">
                  <!-- SVG Inline pour "Stock" -->
                  <svg class="mx-auto mb-1 text-gray-500 w-7 h-7 dark:text-gray-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                      <!-- Exemple d'icône de "Stock" -->
                      <path d="M2 3h16v2H2V3zm0 4h16v2H2V7zm0 4h16v2H2v-2zm0 4h16v2H2v-2z"></path>
                  </svg>
                  <div class="text-sm font-medium text-gray-900 dark:text-white">Stock</div>
              </a>

              <a href="<?= base_url('unit') ?>" class="block p-4 text-center rounded-lg hover:bg-gray-100 dark:hover:bg-gray-600">
                  <svg class="mx-auto mb-1 text-gray-500 w-7 h-7 dark:text-gray-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                      <path d="M4 3a2 2 0 100 4h12a2 2 0 100-4H4z"></path>
                      <path fill-rule="evenodd" d="M3 8h14v7a2 2 0 01-2 2H5a2 2 0 01-2-2V8zm5 3a1 1 0 011-1h2a1 1 0 110 2H9a1 1 0 01-1-1z" clip-rule="evenodd"></path>
                  </svg>
                  <div class="text-sm font-medium text-gray-900 dark:text-white">Unité</div>
              </a>

              <a href="<?= base_url('logout') ?>" class="block p-4 text-center rounded-lg hover:bg-gray-100 dark:hover:bg-gray-600">
                <svg class="mx-auto mb-1 text-gray-500 w-7 h-7 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 16l-4-4m0 0l4-4m-4 4h14m-5 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h7a3 3 0 013 3v1"></path></svg>
                  <div class="text-sm font-medium text-gray-900 dark:text-white">Déconnecter</div>
              </a>
            </div>
          </div>
          ...
          <div class="flex items-center ml-3">
            <div>
              <button type="button" class="flex text-sm bg-gray-800 rounded-full focus:ring-4 focus:ring-gray-300 dark:focus:ring-gray-600" id="user-menu-button-2" aria-expanded="false" data-dropdown-toggle="dropdown-2">
                ...
                <img class="w-8 h-8 rounded-full" src="<?= base_url('assets/images/logo/logo.png') ?>" alt="user photo">
              </button>
            </div>
            <!-- Dropdown menu -->
            <div class="z-50 hidden my-4 text-base list-none bg-white divide-y divide-gray-100 rounded shadow dark:bg-gray-700 dark:divide-gray-600" id="dropdown-2">
              ...
              <ul class="py-1" role="none">
                <li>
                  <a href="<?= base_url('logout') ?>" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-600 dark:hover:text-white" role="menuitem">Déconnecter</a>
                </li>
              </ul>
            </div>
          </div>
...
document.getElementById('mark-read-btn').addEventListener('click', function() {
    ...
    .then(data => {
        if (data.success) {
            ...
            document.querySelector('.absolute span').innerText = '0';
        } else {
    ...
    button.addEventListener('click', function() {
        ...
        fetch(`/notifications/delete/${notificationId}`, {
            ...
            headers: {
                ...
                'X-CSRF-TOKEN': '<?= csrf_token() ?>', // Protection CSRF si activée
            }
        })
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                ...
                // Optionnel : retirer la notification de l'affichage après suppression
